...
march.1812.plot <- ggmap(march.1812.ne.europe.map) +
  ...
  geom_text_repel(data = cities, aes(x = long, y = lat, label = city),
                  color = "#DC5B44", family = "Courier New" ) + #"Open Sans Condensed Bold"
...
ggplot(covid, aes(x=data)) + 
  ggtitle("Morte por Covid-19")+
  geom_line( aes(y=n_obito_log), group = 1, size = 1.3, color = to) +
  geom_point( aes(y=n_obito_log), size = 2, color = to)+
  geom_line( aes(y=taxa_de_crescimento * coeff), group = 1, size = 1.3, color = no) + 
  geom_point( aes(y=taxa_de_crescimento * coeff), size = 2, color = no)+
  scale_y_continuous(name = "Número de óbitos (Log)",sec.axis = sec_axis(~./coeff, name="Taxa de crescimento"))+ 
  theme_minimal()+
  scale_color_manual(labels = c("Taxa de crescimento", "N° de óbitos"), values = c("blue", "red"))+
